...
void setup() {
  ...
  // interrupts DS3231
  pinMode(2, INPUT);
  attachInterrupt(0, onInt, FALLING); 
}
...
void loop() {
    ...
    if (check) {
      if (rtc.isAlarmActive())
        alarm = true;
      else if (rtc.isTickActive())
        tick = true;
      else if (rtc.getTickMode() == DS1337_TICK_EVERY_SECOND)
        tick = true;
      check = false;
    }
    if (alarm) {
  ...
  while (Serial.available()) {
    ...
    // analyze input
      ...
      switch (cmd) {
        ...
        case 's': rtc.toggle32kHz(); updated = true; break;
      }
      if (updated)
        showInfo();
      updated = false;
...
void showHelp() {
  ...
  Serial.println(F("s ............................... toggle 32kHz signal"));
  Serial.println();
}
